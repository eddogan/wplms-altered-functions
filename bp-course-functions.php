...
if(!function_exists('the_question')){
  function the_question(){
    ...
    echo '<div class="question '.$type.'">';
                   if ( get_field( 'comp_quest' ) ){
			echo '<div class="passage">'; 
			echo get_field ('comp_question_here');
			echo '</div>';
			the_content();
		   } else { 
			the_content();
	...
	/*	$img = 0;
      ...
      if($type=='single'){
        foreach($options as $key=>$value){
		  ...
		  if(@getimagesize($value) && $img==1){
			  $image_exit = '<img src="'.$value.'" style="width: 100%;padding-top:10px;"/>';
			   echo '<li class="col-md-4" style="text-align: center; float: left;">
                    <input type="radio" id="'.$post->post_name.$key.'" name="'.$post->post_name.'" value="'.$k.'" '.(in_array($k,$content)?'checked':'').'/>
                    <label for="'.$post->post_name.$key.'"><span></span>'.do_shortcode($image_exit).'</label>
                </li>';
				if($k%3==0){
			   ...
			   if(@getimagesize($value)){
			  		$image_exit = '<img src="'.$value.'" style="width: 100%;padding-left:10px;"/>';
			   }else{
				   $image_exit = $value;
			   }
			   echo '<li>
        ...
        foreach($options as $key=>$value){
			...
			if(@getimagesize($value) && $img==1){
			  $image_exit = '<img src="'.$value.'" style="width: 100%;padding-top:10px;"/>';
			   echo '<li class="sort_option col-md-4" style="text-align: center; float: left;">
                    <label for="'.$post->post_name.$key.'"><span></span>'.do_shortcode($image_exit).'</label>
                </li>';
				if($k%3==0){
        ...
        foreach($options as $key=>$value){
			...
			if(@getimagesize($value) && $img==1){
			  $image_exit = '<img src="'.$value.'" style="width: 100%;padding-top:10px;"/>';
			   echo '<li class="match_option col-md-4" style="text-align: center; float: left;">
                    <label for="'.$post->post_name.$key.'"><span></span>'.do_shortcode($image_exit).'</label>
                </li>';
				if($k%3==0){
        ...
        foreach($options as $key=>$value){
			...
			if(@getimagesize($value) && $img==1){
			  ...
			  if(@getimagesize($value)){
			   ...
			   echo '<li>
                    <input type="radio" id="'.$post->post_name.$key.'" name="'.$post->post_name.'" value="'.$key.'" '.(in_array($key,$content)?'checked':'').'/>
                    <label for="'.$post->post_name.$key.'"><span></span> '.$image_exit.'</label>
                </li>';
		  }
      ...
      if($type=='single'){
        foreach($options as $key=>$value){
          ...
          $k=$key+1;
			   ...
			   echo '<li class="col-md-4" style="text-align: center; float: left;">
                    <input type="radio" id="'.$post->post_name.$key.'" name="'.$post->post_name.'" value="'.$k.'" '.(in_array($k,$content)?'checked':'').'/>
                    <label for="'.$post->post_name.$key.'"><span></span>'.html_entity_decode($value).'</label>
                </li>';
				if($k%3==0){
...
if(!function_exists('the_quiz')){
  ...
  extract( $r, EXTR_SKIP );
    ...
    if($ques_id){
	  ...
	  echo '<a href="#" class="ques_link right quiz_question" data-quiz="'.$quiz_id.'" data-qid="'.$questions['ques'][($key+1)].'">'.__('Save Answer','vibe').' &rsaquo;</a>';

        }elseif($key == (count($questions['ques'])-1)){ // Mark For Reveiw
          echo '<a href="#" class="ques_link right quiz_question  mark" data-quiz="'.$quiz_id.'" data-qid="'.$questions['ques'][($key)].'">'.__(' Mark For Review ','vibe').' &rsaquo;</a>';
	 ...
	 echo '<a href="#" class="ques_link right quiz_question mark" data-quiz="'.$quiz_id.'" data-qid="'.$questions['ques'][($key)].'">'.__(' Mark For Review ','vibe').' &rsaquo;</a>';
      ...
      wp_reset_postdata();
        ...
        if(isset($quiz_taken) && $quiz_taken && ($quiz_taken < time())){
		  ...
		  $total_markes=get_post_meta($quiz_id,$user_id,true);
		  $ques_count=get_post_meta($quiz_id,'vibe_quiz_questions',false);
		  $tques_count = count($ques_count[0]['ques']);
		 // print_r($ques_count);
		 $markes_count = array_sum($ques_count[0]['marks']);
		  
		  //$mark_per = ((100*$total_markes)/$markes_count);
		  $mark_per = $total_markes;
		  
		 // $msg = get_option('wplms');
		  $msg = $message;
		  $val_count = (count($msg)/3);
		  for($k=0;$k<$val_count;$k++){
				$i=$k*3;
				if($mark_per >= $msg[$i] && $mark_per <= $msg[$i+1]){
					$message = $msg[$i+2];
					break;
				}
  ...
  function quiz_question(){
      ...
      if ( isset($_POST['start_quiz']) && wp_verify_nonce($_POST['start_quiz'],'start_quiz') ){ // Same NONCE just for validation
        ...
        echo '<p>'.__('If you want to attempt again, Contact Instructor to reset the quiz.','vibe').'</p></div>';
            ...
            echo '<div class="message info"><h3>'.__('Start Test to begin test.','vibe').'</h3>'; 
            echo '<p>'.__('Click "Start Test" button to start the test.','vibe').'</p></div>';
         }

     }else{
                echo '<div class="message error"><h3>'.__('Test not active.','vibe').'</h3>'; 
